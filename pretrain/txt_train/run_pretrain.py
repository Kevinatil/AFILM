...
from src.dataset import create_bert_dataset, create_eval_dataset, create_csv_dataset
from src.utils import LossCallBack, BertLearningRate, EvalCallBack, BertMetric, LogObsCallBack, log_obs_copy
...
def InitNetWithGrads(net_with_loss, optimizer):
    '''init net with grads'''
    assert cfg.enable_lossscale == "true"
...
def run_pretrain():
    ...
    if not cfg.data_type == "csv":
        ...
        ds = create_csv_dataset(device_num=device_num, rank=rank, do_shuffle=cfg.do_shuffle, data_dir=cfg.data_dir,
                                schema_dir=cfg.schema_dir, batch_size=cfg.batch_size,
                                bucket_list=cfg.bucket_list)
    print('create data done! use time: ',time.time()-btime)

    net_with_loss = BertNetworkWithLoss(bert_net_cfg, is_training=True)
    ...
    log_callback = LogObsCallBack("/tmp/log/", cfg.train_url, rank, cfg.log_step_freq)
    callback = [TimeMonitor(cfg.data_sink_steps), LossCallBack(ds.get_dataset_size()),log_callback]
    if cfg.enable_save_ckpt == "true" and cfg.device_id % min(8, device_num) == 0:
        ...
        ckpoint_cb = ModelCheckpoint(prefix='ab_ckpt',
    ...
    if cfg.train_with_eval == 'true':
        net_eval = BertPretrainEval(bert_net_cfg, network=net_with_loss.predictor)
    ...
    try:
        log_obs_copy("/tmp/log/", cfg.train_url, rank)
    except Exception as e:
        print('final log_obs_copy failed!')

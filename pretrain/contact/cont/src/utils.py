...
def log_obs_copy(log_dir, train_url, rank):
    try:
        print('(in log_obs_copy) from {} to {}'.format(log_dir,train_url))
        print('in log_dir: ',os.listdir(log_dir))
        if os.path.exists(log_dir):
            mox.file.copy_parallel(src_url=log_dir, dst_url=train_url, threads=0, is_processing=False)
    except Exception as e:
        print(f"Exception in logs_to_obs: {e} ")


class LogObsCallBack(Callback):
    def __init__(self, log_dir, train_url, local_rank, freq=1000):
        super(LogObsCallBack, self).__init__()
    ...
    def step_end(self, run_context):
        ...
        print('cur step num',cur_step_num)
        if self.local_rank % 8 == 0 and cur_step_num % self.freq == 0:
            print('process log callback, step: {}, freq: {}'.format(cur_step_num,self.freq))
            p = Process(target=log_obs_copy, args=(self.log_dir, self.train_url, self.local_rank))
            p.start()
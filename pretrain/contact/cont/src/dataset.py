...
def create_bert_dataset(device_num=1, rank=0, do_shuffle="true", data_dir=None, schema_dir=None, batch_size=32,
    ...
    if dataset_format == "mindrecord":
        if str(num_samples).lower() != "none":
            data_set = ds.MindDataset(data_files,
                                      columns_list=["input_ids", "input_mask", "token_type_ids", 
                                                    "cont_label", "cont_label_weight","seq_len"],
    ...
    data_set = data_set.map(operations=type_cast_op, input_columns="cont_label")
    data_set = data_set.map(operations=type_cast_op, input_columns="seq_len")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_mask")
    data_set = data_set.map(operations=type_cast_op, input_columns="input_ids")
    data_set = data_set.map(operations=type_cast_op, input_columns="token_type_ids")
...
def create_eval_dataset(batchsize=32, device_num=1, rank=0, data_dir=None, schema_dir=None,
        ...
        if str(num_samples).lower() != "none":
            data_set = ds.MindDataset(data_files,
                                      columns_list=["input_ids", "input_mask", "masked_lm_positions", 
                                                    "masked_lm_ids", "masked_lm_weights","token_type_ids"],
    ...
    if padded_num > 0:
        item = {
            ...
            "token_type_ids":np.zeros(shapes[5],dtypes[5])
        }
    ...
    eval_ds = eval_ds.map(operations=type_cast_op, input_columns="token_type_ids")

...
def run_pretrain():
    ...
    ckpt_callback = LogObsCallBack(cfg.output_path, cfg.train_url, rank, cfg.log_step_freq)
    callback = [TimeMonitor(cfg.data_sink_steps), LossCallBack(ds.get_dataset_size()), log_callback, ckpt_callback]
    if cfg.enable_save_ckpt == "true" and cfg.device_id % min(8, device_num) == 0:
        print('checkpoint config, save_checkpoint_steps:',cfg.save_checkpoint_steps, ckpt_save_dir)
        config_ck = CheckpointConfig(save_checkpoint_steps=cfg.save_checkpoint_steps,
                                     keep_checkpoint_max=cfg.save_checkpoint_num)
        ckpoint_cb = ModelCheckpoint(prefix='cont_ckpt',
                                     directory=None if ckpt_save_dir == "" else ckpt_save_dir, config=config_ck)
        callback.append(ckpoint_cb)


    if cfg.load_checkpoint_path:
        print('checkpoint path:',cfg.load_checkpoint_path)
        param_dict = load_checkpoint(cfg.load_checkpoint_path)
        not_loaded=load_param_into_net(net_with_loss, param_dict)

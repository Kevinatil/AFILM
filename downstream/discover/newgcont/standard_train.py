...
if __name__ == '__main__':
    ...
    res_dir = "cls_result_{}_lr{}_k{}_f1_".format(BATCH_SIZE,lr,k) + backbone_name
    ...
    for i_ in range(k):
        ...
        for j in range(k):
            if j==i_:
                val_feats=np.load('../../../data/discover/{}_all_data_{}.npy'.format(backbone_name,j))
                val_cls_label=np.load('../../../data/discover/{}_all_label_{}.npy'.format(backbone_name,j))
            else:
                train_feats.append(np.load('../../../data/discover/{}_all_data_{}.npy'.format(backbone_name,j)))
                train_cls_label.append(np.load('../../../data/discover/{}_all_label_{}.npy'.format(backbone_name,j)))

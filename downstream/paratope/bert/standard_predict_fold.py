...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 256
    backbone_name = 'bert' #'T5-XL-UNI' #"T5-XL-UNI" 'BERT' 'T5-XL-UNI'
    model_dir="result_256_lr3e-05_k5_f1_bert/model_{}.pth.tar" #调整为模型路径
    ...
    if 1: #通过训练数据对测试数据进行标准化
        ...
        train_feats=[]
        for i in range(5):
            train_feats.append(np.load('../../../data/paratope/bert_train_data_{}.npy'.format(i)))
        ...
        # load test data
        test_feats=np.load('../../../data/paratope/bert_test_data.npy') #测试数据
        test_cls_label=np.load('../../../data/paratope/bert_test_label.npy')

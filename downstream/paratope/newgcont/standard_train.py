...
class HuggingfaceClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self._dropout(x)
        x = self._fc2(x)
        return self.sigmoid(x) #F.softmax(x, dim=1)
...
if __name__ == '__main__':
    ...
    for i_ in range(k):
        ...
        model = HuggingfaceClassifier(1024, 1) #SoluModel(SEQ_LEN)
        ...
        for j in range(k):
            if j==i_:
                val_feats=np.load('../../../data/paratope/{}_train_data_{}.npy'.format(backbone_name,j))
                val_cls_label=np.load('../../../data/paratope/{}_train_label_{}.npy'.format(backbone_name,j))
            else:
                train_feats.append(np.load('../../../data/paratope/{}_train_data_{}.npy'.format(backbone_name,j)))
                train_cls_label.append(np.load('../../../data/paratope/{}_train_label_{}.npy'.format(backbone_name,j)))
        train_feats=np.concatenate(train_feats, axis=0)
        train_cls_label=np.concatenate(train_cls_label, axis=0)
        
        print(train_feats.shape,train_cls_label.shape)
        print(val_feats.shape,val_cls_label.shape)
        ...
        for epoch in range(EPOCH):
            ...
            for step, (feat_, cls_label_) in enumerate(train_loader_single):
                ...
                #cls_output=cls_output[:,:,1].squeeze()
                cls_output=cls_output.squeeze()
                
                mask_=(cls_label_>=0)

                loss = cls_loss_func(cls_output[mask_], cls_label_[mask_])
            ...
            with torch.no_grad():
                ...
                for step, (feat_, cls_label_) in enumerate(val_loader_single):
                    ...
                    if device.type=='cpu':
                        ...
                        #cls_output=cls_output[:,:,1].cpu().data.numpy().squeeze()
                ...
                mask_=(val_cls_label>=0)
                val_cls_label_=val_cls_label[mask_]
                y_cls_bool=y_cls_bool[mask_]
                y_cls=y_cls[mask_]
                
                cm_=confusion_matrix(val_cls_label_,y_cls_bool)
                vc_=(cm_[0][0]+cm_[1][1])/(cm_[0][0]+cm_[0][1]+cm_[1][0]+cm_[1][1])
                precision_=precision_score(val_cls_label_,y_cls_bool) #cm_[1][1]/(cm_[0][1]+cm_[1][1])
                recall_=recall_score(val_cls_label_,y_cls_bool) #cm_[1][1]/(cm_[1][0]+cm_[1][1])
                f1_=f1_score(val_cls_label_,y_cls_bool)
                auc_=roc_auc_score(val_cls_label_,y_cls)

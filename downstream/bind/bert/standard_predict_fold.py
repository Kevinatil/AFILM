...
if __name__ == '__main__':
    ...
    if 1: #通过训练数据对测试数据进行标准化
        # load train data
        train_feats=np.load('../../../data/bind/bert_train_data_max.npy') #训练数据，用来进行测试数据归一化与标准化
        scaler=StandardScaler()
        scaler.fit(train_feats)
        del train_feats

        # load test data
        test_feats=np.load('../../../data/bind/bert_test_data_max.npy') #测试数据
        test_cls_label=np.load('../../../data/bind/bert_test_label.npy')

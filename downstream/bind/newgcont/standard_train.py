...
if __name__ == '__main__':
    ...
    #feats=np.load('../../../data/bind/{}_train_data.npy'.format(backbone_name))
    pickle_data=pickle.load(open('../../../data/bind/{}_train_sequence.txt_embedding.pickle'.format(backbone_name),
                                    'rb'))
    #feats=[]
    #for data_ in pickle_data:
    #    print(data_.shape)
    #    feats.append(data_[0,:])
    #feats=np.array(feats)
    feats=np.max(np.array(pickle_data),axis=1)
    cls_label=np.load('../../../data/bind/newg_train_label.npy'.format(backbone_name))
